...
Continue reading this page to learn the following:

[](!toc)

Or go directly to the subpages for details about:
...
<th>
```
Data is kept safe and backed up by
```{=html}
</th>
```
```{=html}
...
<td>
```
Magnolia light modules added by customer
```{=html}
</td>
```
```{=html}
...
(JCR data from the workspaces `configuration`, `users`, `userroles`,
`usergroups`)
```{=html}
</td>
```
...
(JCR data in content workspaces such as, `website`, `dam`, `category`,
`contacts`, etc.)
```{=html}
</td>
```
